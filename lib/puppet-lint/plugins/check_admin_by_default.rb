      ...
      tokens.each do |indi_token|
         ...
         if (!nxt_token.nil?) && (!indi_token.nil?)
            ...
            if  (!nxt_nxt_token.nil?)
               ...
               if (token_type.eql? 'NAME') || (token_type.eql? 'VARIABLE') || (token_type.eql? 'SSTRING') || (token_type.eql? 'STRING')
                  nxt_typ = nxt_token.type.to_s
                  nxt_nxt_typ=nxt_nxt_token.type.to_s
                  if (token_line==nxt_tok_line) && (token_line==nxt_nxt_tok_lin)
                     if (token_type.eql? 'VARIABLE') && (nxt_typ.eql? 'EQUALS') && (['STRING', 'SSTRING'].include? nxt_nxt_typ)
                          ...
                          if ((nxt_nxt_val.include? 'admin') && (token_val.include? 'user') && !(nxt_nxt_val =~ /^\${.*}$/)) ||
                             ((token_val.include? 'admin') && (token_val.include? 'user') && !(nxt_nxt_val =~ /^\${.*}$/))
                             notify :warning, {
